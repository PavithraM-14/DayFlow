...
import { checkIn, checkOut, myAttendance } from '@/services/attendance';
...
export default function EmployeeAttendance() {
  ...
  const totalHours = records.reduce((sum, r) => sum + (r.workMinutes || 0), 0);
  const hoursLabel = `${Math.floor(totalHours / 60)}h ${totalHours % 60}m`;
  ...
  const handleCheckIn = async () => {
    ...
    setBusy(false);
    if (response.success) load({ quiet: true });
      ...
      <section className="grid grid-cols-1 md:grid-cols-12 gap-gutter mb-gutter">
        <div className="md:col-span-8 bg-surface-container-lowest border border-outline-variant rounded-xl p-6">
          <div className="flex justify-between items-start mb-4">
            <div>
              <h2 className="font-title-md text-title-md text-on-surface">This month</h2>
        ...
        <div className="md:col-span-4 bg-surface-container-lowest border border-outline-variant rounded-xl p-6">
          <div className="font-label-sm text-label-sm text-on-surface-variant uppercase mb-1">Hours Logged (Month)</div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="font-body-md text-body-md text-on-surface">
              {loading ? (
                <tr><td className="p-4 text-on-surface-variant text-center" colSpan={5}>Loading…</td></tr>
              ) : records.length === 0 ? (
                <tr><td className="p-4 text-on-surface-variant text-center" colSpan={5}>No attendance recorded yet.</td></tr>
              ) : (
                records.map((record) => (

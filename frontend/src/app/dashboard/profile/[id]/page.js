...
export default function EmployeeProfileViewPage() {
  ...
  const saveInterests = async (next) => {
    setSavingTags(true);
    setNotice('');
    const response = await updateEmployee(id, { interests: next });
    setSavingTags(false);
    if (response.success) {
      setInterests(response.data.employee.interests || []);
      setNotice('Interests updated.');
    } else {
      setError(response.message || 'Could not save that.');
    }
  };
  ...
  const card = 'bg-surface-container-lowest rounded-xl border border-outline-variant p-6';
  ...
  return (
    <DashboardShell variant="hr">
      ...
      {/* Tabs */}
      <div className="flex gap-1 mt-gutter mb-gutter border-b border-outline-variant overflow-x-auto">
        {TABS.map((t) => (
          <button
            key={t.key}
            type="button"
            onClick={() => setTab(t.key)}
            className={`px-4 py-2 font-label-sm text-label-sm border-b-2 -mb-px whitespace-nowrap transition-colors ${
              tab === t.key
        ...
        <div className="flex flex-col gap-gutter">
          <div className={card}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-title-md text-title-md">About</h3>
              {!editingProfile && (
                <button type="button" onClick={() => { setTab('private'); setEditingProfile(true); }} className="font-label-sm text-label-sm text-primary hover:underline flex items-center gap-1">
                  <span className="material-symbols-outlined text-[16px]">edit</span> Edit in Private Info
                </button>
              )}
            </div>
            <p className="font-body-sm text-body-sm text-on-surface">{employee?.about || 'No summary added yet.'}</p>
          </div>
          ...
          <div className={card}>
            ...
            <TagListEditor
              value={skills}
              onChange={(next) => { setSkills(next); saveTags(next, certifications); }}
          ...
          <div className={card}>
            ...
            <TagListEditor
              ...
              onChange={(next) => { setCertifications(next); saveTags(skills, next); }}
          ...
          <div className={card}>
            ...
            <TagListEditor
              value={interests}
              onChange={(next) => { setInterests(next); saveInterests(next); }}
          ...
          <div className="lg:col-span-1 flex flex-col gap-gutter">
            ...
            <div className={card}>
              ...
              <div className="grid grid-cols-2 gap-3 font-body-sm text-body-sm">
                <Field label="Paid / year" value={`${employee?.leaveAllocation?.paid ?? 24} days`} />
                <Field label="Sick / year" value={`${employee?.leaveAllocation?.sick ?? 7} days`} />
              </div>
            </div>
          ...
          <div className="lg:col-span-2">
            <div className={card}>
              <div className="flex items-center justify-between mb-4">
                ...
                {!editingProfile && (
                  <button type="button" onClick={() => setEditingProfile(true)} className="font-label-sm text-label-sm text-primary hover:underline flex items-center gap-1">
                    <span className="material-symbols-outlined text-[16px]">edit</span> Edit
                  </button>
                )}
              </div>
              ...
              {!editingProfile ? (
                ...
                <form onSubmit={saveProfile} className="flex flex-col gap-4">
                  ...
                  <div className="flex justify-end gap-3">
                    <button type="button" onClick={() => setEditingProfile(false)} className="px-4 py-2 rounded-lg font-label-sm text-label-sm text-on-surface-variant hover:bg-surface-container-highest">Cancel</button>
      ...
      {tab === 'salary' && (
        <div className={card}>
          <div className="flex items-center justify-between mb-4">
            ...
            {!editingSalary && (
              <button type="button" onClick={() => setEditingSalary(true)} className="font-label-sm text-label-sm text-primary hover:underline flex items-center gap-1">
                <span className="material-symbols-outlined text-[16px]">edit</span> Edit
              </button>
            )}
          </div>
          ...
          {editingSalary ? (
            <form onSubmit={saveSalary} className="flex flex-col gap-5">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                <TextInput type="number" label="Monthly Wage (₹)" value={salaryForm.wage} onChange={(v) => setSalaryForm({ ...salaryForm, wage: v })} />
                <TextInput type="number" label="Working Days / Week" value={salaryForm.workingDaysPerWeek} onChange={(v) => setSalaryForm({ ...salaryForm, workingDaysPerWeek: v })} />
                <TextInput type="number" label="Break Time (mins)" value={salaryForm.breakTimeMinutes} onChange={(v) => setSalaryForm({ ...salaryForm, breakTimeMinutes: v })} />
              </div>
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                  {COMPONENT_FIELDS.map(({ key, label, unit }) => (
                    <TextInput key={key} type="number" label={`${label} (${unit})`} value={salaryForm[key]} onChange={(v) => setSalaryForm({ ...salaryForm, [key]: v })} />
                  ))}
                </div>
              ...
              <div className="flex justify-end gap-3">
                <button type="button" onClick={() => setEditingSalary(false)} className="px-4 py-2 rounded-lg font-label-sm text-label-sm text-on-surface-variant hover:bg-surface-container-highest">Cancel</button>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 font-body-sm text-body-sm">
              ...
              <Field label="Yearly Wage" value={currency(salary.wage * 12)} />
              <Field label="Working Days / Week" value={salary.workingDaysPerWeek ?? '—'} />
      ...
      {tab === 'security' && (
        <div className={card}>
          <h3 className="font-title-md text-title-md mb-2">Security</h3>
          <p className="font-body-sm text-body-sm text-on-surface-variant">
            Passwords are private — each person changes their own from the Security tab on their profile. If this
            employee is locked out, they can reset it from the sign-in page, or you can re-create their access from the
            directory.
          </p>
        </div>
      )}
